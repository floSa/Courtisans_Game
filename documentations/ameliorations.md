...
| 8  | IA / MCTS       | `clone_determinized` triche (information parfaite)                    | 🟠       | ❌ non (gros chantier, voir note) |
...
### #8 — Vraie déterminisation MCTS (information imparfaite)

**Contexte.** Courtisans est un jeu à information imparfaite :
- la main de l'adversaire est cachée ;
- les espions posés face cachée chez l'adversaire sont invisibles ;
- l'ordre exact de la pioche restante est inconnu.

Or `clone_determinized` fait `copy.deepcopy(self)` : la simulation a accès à
toutes ces informations. L'IA s'entraîne donc avec une information qu'un humain
n'aurait pas → elle apprend des stratégies non transférables.

**Pistes.**
1. **PIMC (Perfect Information Monte Carlo)** : à chaque appel de
   `clone_determinized`, lister les cartes inconnues du point de vue du joueur
   courant (`unseen_pool`), les redistribuer aléatoirement à l'adversaire et à
   la pioche, et tirer plusieurs déterminisations (5–20). Moyenner les visites
   MCTS sur l'ensemble.
2. **IS-MCTS (Information Set MCTS)** : on garde un seul arbre indexé par
   *information set* du joueur courant, et on redéterminise à chaque
   simulation.

PIMC est le plus simple à brancher sur l'architecture actuelle.
