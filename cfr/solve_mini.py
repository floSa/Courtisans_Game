...
def main():
    ...
    # Structure du jeu : états et info-sets par joueur.
    ...
    print(f"États : {len(allst)} | terminaux : {n_term} | "
          f"info-sets P0 : {len(infosets[0])} | P1 : {len(infosets[1])}")

    solver = cfr.CFRPlusSolver(game)
    for i in range(1, ITERS + 1):
        solver.evaluate_and_update_policy()
        if i in (1, 10, 50, 100, 300, ITERS):
            nc = exploitability.nash_conv(game, solver.average_policy(), use_cpp_br=False)
            print(f"CFR+ iter {i:4d} : NashConv={nc:.6f}  exploitabilité={nc/2:.6f}")
